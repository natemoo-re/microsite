...
async function writeGlobal() {
    const global = await rollup({ ...internalRollupConfig, plugins: [...requiredPlugins, typescript({ target: 'ES2018' }), ...globalPlugins], input: 'src/global.ts' });
    const legacy = await rollup({ ...internalRollupConfig, plugins: [...requiredPlugins, typescript({ target: 'ES5' }), ...globalPlugins], input: 'src/global.ts' });
...
async function writePages() {
    try {
        ...
        const bundles = await Promise.all(pages.map(input => rollup({ ...internalRollupConfig, plugins: [...requiredPlugins, typescript({ target: 'ES2018' }), ...createPagePlugins()], input })));
