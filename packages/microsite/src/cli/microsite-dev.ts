import { startServer, SSRLoader } from "snowpack";
import arg from "arg";
...
const doc = async (
  ...
  let Doc = null;
  try {
    Doc = await runtime.importModule('/src/pages/_document.js');
  } catch (e) {
    console.error(e);
  }
  console.log('custom document?', Doc);
  if (!Doc) {
    try {
      Doc = await runtime.importModule('/web_modules/microsite/document.js');
    } catch (e) {
      console.error(e);
    }
  }
  console.log('built-in document?', Doc);
