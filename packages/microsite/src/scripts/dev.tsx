...
const pageScript = (page: string, props?: any) => `import { csr } from '/web_modules/microsite/runtime/csr.js';
import Page from '${page}';
csr(${props ? `Page, ${JSON.stringify(props)}` : `Page`});`;

const errorScript = (props: any) => `import { h, render } from '/web_modules/preact.js';
...
const errorPage = (props: any) => `<!doctype html>\n<!-- Generated by microsite -->\n<html lang="en" dir="ltr">\n\t<head></head>\n\t<body>
\t\t<div id="__microsite"></div>
\t\t<script data-csr="true">window.HMR_WEBSOCKET_URL = 'ws://localhost:3333';</script>
\t\t<script type="module" src="/__snowpack__/hmr-client.js"></script>
...
export async function dev({ port: PORT = 8888 } = {}) {
    const cwd = process.cwd();
    
    const [errs, config] = createConfiguration(_config);
    if (errs) {
    ...
    config.alias = { 'microsite/hydrate': 'microsite/runtime/hydrate' }
    ...
    const loadErrorPage = async (contentType: string, props?: ErrorProps) => {
        try {
            ...
            if (contentType === 'text/html') return doc('/src/pages/_error.js', props);
            if (contentType === 'application/javascript') return result.contents.toString();
        } catch (e) {}
    ...
    const server = polka()
        ...
        .use(async (req: IncomingMessage, res: ServerResponse, next: any) => {
            ...
            const findPotentialMatch = async (base: string) => {
                ...
                files = files
                    ...
                    .map((file: string) => file.slice(pages.length, extname(file).length * -1))
                    .filter((file: string) => {
                        if (file.indexOf('[') === -1) return false;
                        const parts = file.slice(1).split('/');
                        if (parts.length === baseParts.length - 1) return parts.every((part, i) => part.indexOf('[') > -1 ? true : part === baseParts[i]);
                        if (parts.length === baseParts.length) return parts.every((part, i) => part.indexOf('[') > -1 ? true : part === baseParts[i]);
                        if (file.indexOf('[[') > -1) return parts.every((part, i) => {
        ...
        .use(async (req: IncomingMessage, res: ServerResponse, next: any) => {
            try {
                ...
                if (req.url.indexOf('/web_modules/microsite') === -1) {
                    result.contents = result.contents.toString().replace(/preact\/hooks/, 'microsite/runtime/hooks');
                }
        ...
        .get('*', (_req: IncomingMessage, res: ServerResponse) => sendErr(res, { statusCode: 404 }));
